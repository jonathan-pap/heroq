...
| `client/furniture-art.js` | `HQFurnitureArt` | PNG-based **furniture** rendering. Owns canonical-pieces hydration (`/api/canonical-pieces`), furn-naturals overrides (`/api/furn-naturals` + cross-tab sync), the FURN_IMG / FURN_IMG_ALT image caches, the ALT_FURN_ON preference, and the per-art-set furniture inset tables (small / linear / stair / block). `init({ ctx, CELL, getLastView, drawBoard })` once at boot. Public: `isAltOn() / setAltOn(b)`, `getFurnImg(type)`, `insetForBbox(cW, cH)`. |
| `client/tile-art.js` | `HQTileArt` | PNG-based **overlay tile** rendering (rubble / falling-block / trap markers / stairway). Owns canonical-tiles hydration (`/api/canonical-tiles`), the TILE_IMG / TILE_IMG_ALT image caches, and the tile-icon inset table (small / linear / block). Shares the `ALT_FURN_ON` preference with `HQFurnitureArt` via an `isAltOn` callback. `init({ ctx, CELL, getLastView, drawBoard, isAltOn })` once at boot. Public: `drawTileIcon(kind, px, py, pw, ph)`, `tileInsetForBbox(cW, cH)`. |
| `client/lobby.js` | `HQLobby` | Everything visible before "Start quest": quest picker (Quest Book / Sandbox optgroups), intro + board label, auto-roll + reveal-all options, GM-mode radios + claim button, four hero-seat tiles (with Male / Female variant toggle), players list with host / GM / hero / offline tags, spell-draft picker (wizard + elf pick elemental groups), Start button gating. `init({ send, getLastView })` once at boot wires all the form-control listeners; `render(view)` is called from applyState on every `'lobby'` phase view. |
| `client/sidebar.js` | `HQSidebar` | Right-rail panes: Spells (hand cards), Items (inventory), Log (auto-scrolling event feed), and the tab switcher. Counts on the tab buttons + the action-panel open buttons. `init({ getPendingSpell, onSpellClick, action })` once at boot. Public: `renderSpells/renderItems/renderLog/updateTabCounts/setSidebarTab`. |
| `client/board-input.js` | `HQBoardInput` | Canvas hover / click / mouseleave + the floating `.hover-tip` tooltip. Owns cursor-picking (crosshair attack / pointer move / help inspect / not-allowed / default) and dispatches the right server action on click (spell target / trap disarm / attack / move / GM monster pick / GM attack / GM move). `init(...)` once at boot — accepts the canvas + cell-coord helper + getters/setters for the bits of state shared with the renderer (hoverCell, pendingSpell, lastReachable, selectedGMMonsterId). |
| `client/entity-draw.js` | `HQEntityDraw` | Per-entity canvas painters: `drawTreasure / drawSecretDoor / drawTrap / drawHero / drawMonster`. Hero / monster painters prefer the loaded sprite (`HQSprites` caches) and fall back to a coloured circle / diamond + glyph. Trap painter prefers the canonical PNG (`HQFurnitureArt.drawTileIcon`) with a pit-circle / X-cross fallback. Wounded monsters get an HP bar. `init({ ctx, CELL, sprites, drawTileIcon })` once at boot. |
| `client/options.js` | `HQOptions` | UI chrome + the ⚙ menu. Owns three boolean prefs read by the canvas renderer (`floorTexturesOn / lightWallsOn / outerWallsOn` — getter functions), the per-panel collapse state, and the rails-hidden toggle. Cross-tab live sync on the wall prefs via `storage` events. `init({ send, getLastView, drawBoard })` once at boot; `mountPanelCollapse() / applyRailsHidden(b) / mountOptionsMenu()` called from `initGameUIChrome` on first game-screen render; `syncFromView(view)` refreshes the menu toggle dots each render. |
